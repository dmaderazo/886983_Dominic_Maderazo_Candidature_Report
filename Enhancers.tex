...
% \end{itemize}
        ...
        Example of such a method is Change Point~\cite{keith2006segmenting}. To attempt to overcome some of the weaknesses of comparative techniques, Change Point utilizes information about the frequency of nucleotide bases associated with function and transition/transvertion ratio in conjunction with conservation. The program encodes an alignment into a string representing the information discussed. A Bayesian hierarchical model is used to segment the string and allocate segments into distinct groups with homogeneous properties. Since the inception of the program, it has been used to investigate genomic structure and for the identification of TFBS~\cite{algama2014investigating, algama2017genome}. 
        ...
        The basis for the computational identification of TFBSs is as follows: Users typically provide a collections of regions flanking genes that are thought to be simulateneously regulated, computational tools are then used to identify DNA statistically overrespresented sequence motifs. A variety of techniques have been developed for the identification of TFBS~\cite{tompa2005assessing}. It has been shown that TFBS show significantly higher conservation when their target gene is expressed in both species~\cite{hemberg2011conservation}. Positive correlation with combinatorial TF binding and transcription levels of closeby genes~\cite{wong2014decoupling}. However, the results of such computational tools still need additional work to be classified as enhancers. 
